...
def create_gaussian_filter(blur_sigma, channels):
    ...
    gaussian_filter = nn.Conv2d(channels, channels, kernel_size=kernel_size, padding=bs_round, groups=channels, bias=False)
...
class MyUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        score = torch.sigmoid(self.avg(x4)) # value in range [0, 1]
        x5 = self.up1(x4, x3)
        x6 = self.up2(x5, x2)
        x7 = self.up3(x6, x1)
        #output = torch.sigmoid(self.outc(x7)) # mask in range [0, 1]
        output = self.outc(x7)
        return output, score
...
class MyMiniUNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False, blur_sigma=0.0, border_zero=False):
        ...
        self.up1 = MyUp(384, 128, bilinear)
        self.up2 = MyUp(192, 64, bilinear)
        self.up3 = MyUp(96, 32, bilinear)
        self.outc = nn.Conv2d(32, n_classes, kernel_size=1)

        if blur_sigma:
            self.gaussian_filter = create_gaussian_filter(blur_sigma, n_channels)
            # print(gaussian_filter.weight.data) # sums to 1 per channel
