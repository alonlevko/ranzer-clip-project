...
def read_image_robust(img_path, monochromatic=False):
    ...
    try:
        ...
        if raw_image.mode is 'RGBA':
            raw_image = raw_image.convert("L")

        image = np.array(raw_image).copy()  # always uint8

        if raw_image.mode is 'L' and monochromatic is False:
...
class MyCopyPasteDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while not(success):
            ...
            fp_fore = self.all_fore_files[file_idx]
            fore, success = read_image_robust(fp_fore)
            if not(success):
                continue
            if self.has_masks:
                fp_mask = self.all_mask_files[file_idx]
                mask, success = read_image_robust(fp_mask, monochromatic=True)
        ...
        while not(success):
            ...
            fp_back = self.all_back_files[file_idx2]
            back, success = read_image_robust(fp_back)
        ...
        while not(success):
            ...
            fp_irel = self.all_fore_files[file_idx3]
            irrel, success = read_image_robust(fp_irel)
        ...
        gfake_mask = self.mask_tf(create_random_gfake_mask(width, height))
